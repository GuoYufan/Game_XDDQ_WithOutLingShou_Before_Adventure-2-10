...
class Fight():
    def __init__(self,左,右):
        ...
        self._本场结束=False
        self.最大回合数=15
        self.累计触发连击=dict.fromkeys((self.左.名称,self.左.对手.名称),0)
        self.累计触发反击=dict.fromkeys((self.左.名称,self.左.对手.名称),0)
        self.累计触发暴击=dict.fromkeys((self.左.名称,self.左.对手.名称),0)
    ...
    def 进行选手存档(self,左,右):
        ...
        self.左_存档.对手=右
        self.左_存档.对手.对手=左
        for _ in (self.左_存档, self.左_存档.对手):
            _.战场=self
            _.记者.选手=_
            for key in _.buff:
                _.buff[key].role=_
    ...
    def 设置双方(self):
        self.左=copy.deepcopy(self.左_存档)
        self.左.对手=copy.deepcopy(self.左_存档.对手)
        self.左.对手.对手=self.左
